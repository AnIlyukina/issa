...
<template>
  <progress-step :steps="steps" :current-step="authStatus" />
  <auth-form :form-name="getName()">
    <register-step v-if="authStatus === 'registration'" />
    <fill-profile-step v-if="authStatus === 'fillProfile'" />
    <load-photo-step v-if="authStatus === 'loadPhoto'" />
  </auth-form>
</template>
